...
CREATE TABLE tenants (
    ...
    settings JSONB DEFAULT '{}', -- e.g. {"agile_mode": true}
...
CREATE TABLE org_units (
    ...
    parent_id UUID REFERENCES org_units(id), -- Null for Top-Level Regions
    
    name VARCHAR(255) NOT NULL, -- "Wireline - Iraq"
    type VARCHAR(50) NOT NULL, -- 'Region', 'Country', 'Business_Unit', 'Business_Line', 'Team'
...
CREATE TABLE users (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID REFERENCES tenants(id),
    org_unit_id UUID REFERENCES org_units(id), -- Functional Home
    
    email VARCHAR(255) UNIQUE NOT NULL,
    password_hash VARCHAR(255) NOT NULL,
    full_name VARCHAR(100),
    
    role VARCHAR(50) NOT NULL, -- 'Admin', 'Manager', 'Supervisor', 'Technician', 'Storeman', 'Viewer'
...
CREATE TABLE locations (
    ...
    name VARCHAR(255) NOT NULL,
    type VARCHAR(50) NOT NULL, -- 'Site', 'Building', 'Zone', 'Shelf'
...
CREATE TABLE assets (
    ...
    -- Hierarchy & Ownership
    parent_id UUID REFERENCES assets(id), -- Component -> Unit
    location_id UUID REFERENCES locations(id), -- Physical Location
    org_unit_id UUID REFERENCES org_units(id), -- Functional Owner
    
    -- Core Identity
    name VARCHAR(255) NOT NULL,
    status VARCHAR(50) DEFAULT 'Draft', -- 'Draft', 'Active', 'Down', 'Archived', 'Red_Tag'
    ...
    specs JSONB DEFAULT '{}', -- {"voltage": "220v", "rpm": 1500}
...
CREATE TABLE asset_identities (
    ...
    type VARCHAR(50) NOT NULL, -- 'Client_Code', 'QR_Token', 'Serial_Number', 'Barcode'
...
CREATE TABLE checklist_templates (
    ...
    description TEXT,
    items JSONB DEFAULT '[]', -- [{"label": "Check Oil", "type": "pass_fail"}, ...]
    created_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE pm_schedules (
    ...
    -- The "Race to Zero" Logic (NULL means ignore)
    interval_days INT,          
...
CREATE TABLE work_orders (
    ...
    origin VARCHAR(50) NOT NULL, -- 'Manual_Request', 'Preventive_Auto', 'Defect_Followup'
...
CREATE TABLE wo_resource_logs (
    ...
    source VARCHAR(50) DEFAULT 'Warehouse' -- 'Warehouse', 'Tech_Wallet'
);
...
CREATE TABLE attachments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID REFERENCES tenants(id),
    parent_type VARCHAR(50) NOT NULL, -- 'Asset', 'WorkOrder'
